...
func (nt *NewsletterTransport) GetListNewsletterEmails(c echo.Context) error {
	response := types.HTTPResponse{Success: true, Response: ""}
	newsletterEmails, err := nt.usecase.GetNewsletterEmails()
	if err != nil {
		response.Success = false
		response.Response = err.Error()
		return c.JSON(http.StatusBadRequest, response)
	}
	response.Response = newsletterEmails
